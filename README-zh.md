...
默认情况下，每个浏览器仍是相互独立的匿名所有者。对于由 `ACCESS_CODE` 保护的可信单课程库部署，可把 `OPENMAIC_SHARED_OWNER_ID` 设置为一个稳定的非空值，使课程和 Agent 会话库在不同浏览器之间共享。该值必须保持不变：它就是服务端所有者键，修改后会进入一个新的空课程库。这不是逐用户登录或隔离；如果访问者之间互不信任，应改用真正的用户账号体系。

启用服务端持久化后，已经保存了本地课程的浏览器会在下一次加载课程库时把这些课程文档复制到服务端。此迁移覆盖课程文档；在服务端媒体存储启用前生成的媒体字节仍可能只存在原浏览器中，需要重新生成。
